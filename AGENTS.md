...
* Encrypted WordPress define values live in committed project `nf.age`; `nf.json` stores only opaque `secret` references. nf encrypts/decrypts the store directly with the agency age identity derived from `NF_PASSWORD_SALT`; agenix, `secrets.nix`, project `.env.age`, and decrypted project `.env` files are not part of this model.
...
* `nf define list/get/status/sync/set/remove/migrate-env/rekey` manages project `wp-config.php` constants declared in `wordpress.defines`. `sync [remote]` patches local or remote `wp-config.php` with a temp file and atomic replace, never persistent `.bak`/dated backups. It owns only the `/* nf-managed wp-config defines: begin */` to `/* nf-managed wp-config defines: end */` project block; removed `nf.json` entries are pruned from that block on sync, while manual constants outside the block are preserved. Legacy per-define `/* nf-managed wp-config defines */` markers are migrated into the block on sync. Status and list output show define names and sources only, never resolved secret values; `get` deliberately prints one raw value to stdout. Duplicate target constants or configured constants outside the nf-managed project block are treated as unsafe and block sync/repair until manually resolved.
* `wordpress.defines` entries require `name` and exactly one top-level `value`, legacy `env`, or opaque `secret` reference for all envs, or `values` keyed by remote name, canonical env id, env name, `local`, or `default`. Bare `nf define set` picks from every configured define plus `Add a new define...`; incomplete forms prompt for missing input. Existing literals and masked secrets are prepopulated. `nf define set <name> <value>` writes a shared literal. `nf define set <name> --secret` reads a hidden value and encrypts it in `nf.age`; `--secret-stdin` is the non-interactive route. `--for <selector>` writes selector-specific values and promotes an existing shared value to `values.default`. Bare `nf define get` picks from configured defines and fails without opening a picker when none exist. `nf define get [name] [--for <selector>]` outputs only the raw literal, legacy env, or decrypted secret value; selector-backed defines require an exact selector, prompt interactively when omitted, and do not fall back to `default`. Legacy `env` entries remain readable only for migration; new authoring must not add them. `nf define migrate-env` reads project-root `.env` directly, converts legacy mappings while preserving selectors, adds unmatched assignments as same-named shared secret defines, and deletes `.env` only with `--delete-source` after verification. Secret store updates use safe two-file ordering so interrupted writes can leave encrypted orphans but not missing referenced values. Provider-owned constants such as `KINSTAMU_WHITELABEL` are rejected from `nf define`; Kinsta repair manages them.
* `nf define rekey --add-recipient <age1...>` adds a new recipient while the old salt is active. After all project stores are committed for both recipients and the team salt is rotated, `nf define rekey` under the new salt prunes every store back to the current recipient. The old salt is required until the transition is complete.
* `wordpress.plugins` in `nf.json` is an env bootstrap checklist, not a full lifecycle manager. String entries install from wordpress.org, activate, and enable auto-updates by default; object entries require `slug`, support `source`, `install`, `note`, and `auto_update`, and default `install`, `activate`, and `auto_update` to true. Use `install: false` for manual/documentation-only plugins that nf should check but never install. Use `source: "repo"` for project-specific plugin source directories at `plugins/<slug>/`; local envs bind mount configured repo plugins for live development, while remote installs zip/upload and replace them on demand before cleaning up temporary artifacts. Repo-plugin remote status/diff directly compares a deterministic relative-path/content fingerprint of the installed directory with the exact local `packagePluginSource` payload; it must detect changed, missing, and extra files without trusting deploy metadata. Use `source: "cache"` for explicit local cached zips under `$NF_DATA_HOME/plugins/<slug>/<slug>.zip`; cached packages must declare a WordPress plugin version and replace installed copies only when newer. nf does not silently fall back to cache for wordpress.org plugins.
...
* `nf plugin add <plugin>` appends to `wordpress.plugins` in `nf.json`, creates the array if missing, rejects duplicate slugs, and does not install anything. With `--source repo`, it creates a minimal `plugins/<slug>/<slug>.php` scaffold only when the plugin directory is missing. `nf plugin remove <plugin>` removes a configured plugin from `nf.json`, rejects missing slugs, and does not uninstall anything. `nf plugin status [remote]` compares configured plugins against local or remote WordPress state and reports installed, active, and auto-update status plus current/drifted/unavailable code state for repo plugins. `nf plugin diff [remote]` reports needed install/activate/auto-update changes, repo source refreshes, missing manual plugins, and installed plugins that are not configured in `nf.json`; it combines source and state drift, mutates nothing, exits 0 when configured plugins and repo payloads match and no extras are installed, and exits 2 when drift exists. Local repo-plugin status treats an existing installed bind mount as current. Remote inspection stays in one SSH status command and reads installed files directly. `nf plugin install` with no remote targets local env and skips missing local cache/repo sources so other plugins can proceed. `nf plugin install <remote>` targets a configured repo remote, prints a plan, and asks yes/no unless `--yes`; `--dry-run` must not SSH. Remote URL plugin sources must be reachable from the remote host; local zip sources are uploaded to a temporary remote directory and cleaned up after install. Plugin install is source-aware: remote repo plugins always replace the installed copy, regardless of status/diff results; cache plugins replace it only when their declared version is newer, and other sources install only when missing. It activates inactive plugins when requested and enables native WordPress auto-updates only when not already enabled. It must not install manual plugins, update WordPress.org/URL plugins, remove, pin, disable auto-updates, or manage plugin licenses.
* `nf plugin cache pull [plugin] [remote]` and `nf theme cache pull [theme] [remote]` classify installed remote code through the official WordPress.org API. Public items are configured as simple wordpress.org strings without downloading code. Private items are strictly downloaded, packaged into their local `NF_DATA_HOME` cache, and configured as `source: "cache"`; known repo sources are rejected. Missing arguments use pickers.
* `nf plugin pull [plugin] [remote]` and `nf theme pull [remote]` recover private remote code into repo sources. They require a completely clean Git worktree before remote work and again before apply. Pulls overlay remote regular files and preserve local-only files; they reject symlinks and file/directory conflicts and leave changes unstaged. Private non-repo plugins are adopted under `plugins/<slug>`. Theme pull always uses the active remote theme, rejects a mismatch with an existing repo theme, and otherwise adopts the private theme under `theme/` as the first configured theme.
...
* Database exports and snapshots carry the active WordPress table prefix in `table-prefix.txt`. Pull/import/restore updates generated local `WP_TABLE_PREFIX` state and recreates only WordPress before database import; the prefix is environment/artifact state, never `nf.json` project metadata. Push refuses a local/remote prefix mismatch before creating the remote backup or changing remote data.
