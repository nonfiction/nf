...
* `nf theme deploy <remote> [--dry-run]` is a packaged release deploy. It installs configured non-repo themes first when needed, remains one command, does not require manual WP admin upload, and supersedes direct in-place rsync of the source theme. After a Kinsta release switch and rewrite flush, it restarts PHP and clears site cache through the Kinsta API so PHP-FPM cannot retain stale theme bytecode.
...
* `nf theme rollback <remote> [--dry-run]` restores the previous recorded release and does not rebuild or upload artifacts. Kinsta rollback runs the same PHP restart and cache clear as deploy.
