...
`nf define add --secret` uses hidden input; `--secret-stdin` supports automation without a plaintext positional argument. Store rewrites and manifest changes use ordering that can leave only an unreferenced encrypted orphan after interruption. Resolution fails closed before modifying `wp-config.php` when the salt, ciphertext, identity, payload, or referenced value is unavailable or invalid. `nf define migrate-env` converts project-root `.env` assignments and legacy env references only after a complete preflight, and removes `.env` only when explicitly requested after encrypted and manifest readback verification. `nf define rekey --add-recipient` supports a two-recipient transition before `NF_PASSWORD_SALT` rotation; `nf define rekey` under the new salt then removes the old recipient.
