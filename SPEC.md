...
`nf site domain primary --wait` asks for launch approval before waiting, polls the same readiness checks as `nf site domain check`, and runs the primary launch automatically as soon as checks pass. It must not prompt again after checks pass; the value of the flag is unattended wait-then-cutover. If checks never pass before the timeout, it exits without changing primary state.
