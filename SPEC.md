...
* [x] `nf env shell`
* [x] `nf env wp -- <args>`
* [x] `nf define list/get/status/sync/set/remove`
* [x] `nf plugin list`
...
* WordPress plugin bootstrap intent
* WordPress `wp-config.php` define intent, with encrypted values referenced from committed `nf.age`
...
`wordpress.defines` values use exactly one source: a commit-safe literal `value`, an opaque `secret` reference generated by nf, or a persisted legacy `env` reference accepted only for migration. Encrypted values live in the ASCII-armored age file `nf.age` beside `nf.json`. The encrypted payload is versioned, records its recipients, and maps opaque references to values; plaintext values never appear in `nf.json`, command previews, list output, or status output. `nf.age` exists only when at least one encrypted define is configured.

`nf define set --secret` uses hidden input; `--secret-stdin` supports automation without a plaintext positional argument. Bare `set` presents every configured define plus an add-new option. Interactive edits prepopulate existing literal values and masked decrypted secret values without rendering secret defaults visibly. Bare `nf define get` picks from configured defines, while every successful `get` prints exactly one raw literal, legacy env, or decrypted secret value to stdout; selector-backed defines require an exact selector and never silently fall back to `default`. Store rewrites and manifest changes use ordering that can leave only an unreferenced encrypted orphan after interruption. Resolution fails closed before modifying `wp-config.php` when the salt, ciphertext, identity, payload, or referenced value is unavailable or invalid. `nf define migrate-env` converts project-root `.env` assignments and legacy env references only after a complete preflight, and removes `.env` only when explicitly requested after encrypted and manifest readback verification. `nf define rekey --add-recipient` supports a two-recipient transition before `NF_PASSWORD_SALT` rotation; `nf define rekey` under the new salt then removes the old recipient.
