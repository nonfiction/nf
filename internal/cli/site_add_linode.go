...
func renderSiteAddScript(plan siteAddPlan) string {
	...
	b.WriteString("; fi\n")
	b.WriteString("db_access_user=$(jq -r '.db.user // .db.database.user // .adminer.user // .adminer.database.user // \"\"' /var/lib/nf/target.json 2>/dev/null || true)\n")
	b.WriteString("if [[ ! \"$db_access_user\" =~ ^[A-Za-z0-9_-]{1,32}$ ]]; then db_access_user=\"\"; fi\n")
	...
	b.WriteString("  if [ -n \"$db_access_user\" ] && [ \"$db_access_user\" = \"$db_name\" ]; then\n")
	b.WriteString("    printf 'Refusing to create site database user \"%s\" because it matches the target database access user.\\n' \"$db_name\" >&2\n")
	...
	b.WriteString("SQL\n")
	b.WriteString("  if [ -n \"$db_access_user\" ]; then\n")
	b.WriteString("    db_access_user_exists=$(mariadb -N -B -uroot -e \"SELECT COUNT(*) FROM mysql.user WHERE User='$db_access_user' AND Host='localhost';\" 2>/dev/null || true)\n")
	b.WriteString("    if [ \"$db_access_user_exists\" = \"1\" ]; then\n")
	b.WriteString("      mariadb -uroot <<SQL\n")
	b.WriteString("GRANT ALL PRIVILEGES ON \\`$db_name\\`.* TO '$db_access_user'@'localhost';\n")
	b.WriteString("FLUSH PRIVILEGES;\n")
	b.WriteString("SQL\n")
	b.WriteString("    fi\n")
	b.WriteString("  fi\n")
