...
func renderSiteAddScript(plan siteAddPlan) string {
	...
	b.WriteString("  if [ -n \"$adminer_user\" ] && [ \"$adminer_user\" = \"$db_name\" ]; then\n")
	b.WriteString("    printf 'Refusing to create site database user \"%s\" because it matches the target Adminer database user.\\n' \"$db_name\" >&2\n")
	...
	b.WriteString("    adminer_user_exists=$(mariadb -N -B -uroot -e \"SELECT COUNT(*) FROM mysql.user WHERE User='$adminer_user' AND Host='localhost';\" 2>/dev/null || true)\n")
	b.WriteString("    if [ \"$adminer_user_exists\" = \"1\" ]; then\n")
	b.WriteString("      mariadb -uroot <<SQL\n")
	b.WriteString("GRANT ALL PRIVILEGES ON \\`$db_name\\`.* TO '$adminer_user'@'localhost';\n")
	b.WriteString("FLUSH PRIVILEGES;\n")
	b.WriteString("SQL\n")
	b.WriteString("    fi\n")
	b.WriteString("  fi\n")
