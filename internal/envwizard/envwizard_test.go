...
func TestEnsureMissingNonInteractive(t *testing.T) {
	...
	err := Ensure([]Requirement{{Keys: []string{"NF_PASSWORD_SALT"}, Secret: true, Required: true}}, true)
	if err == nil {
		t.Fatal("Ensure() error = nil, want error")
	}
	msg := err.Error()
	if !strings.Contains(msg, "NF_PASSWORD_SALT") {
		t.Fatalf("Ensure() error = %q, want NF_PASSWORD_SALT in message", msg)
	}
...
func TestEnsureDoesNotWriteWhenEnvAlreadySatisfies(t *testing.T) {
	...
	if err := Ensure([]Requirement{{Keys: []string{"NF_PASSWORD_SALT"}, Secret: true, Required: true}}, true); err != nil {
...
func TestEnsureIgnoresMissingOptionalRequirements(t *testing.T) {
	t.Setenv("NF_CONFIG_HOME", t.TempDir())
	if err := Ensure([]Requirement{{Keys: []string{"EXAMPLE_ACCOUNT_ID"}, Default: "14"}}, true); err != nil {
		t.Fatalf("Ensure() error = %v", err)
	}
}
...
func TestInitNonInteractiveWritesOptionalDefaults(t *testing.T) {
	t.Setenv("NF_CONFIG_HOME", t.TempDir())
	if err := Init([]Requirement{{Keys: []string{"EXAMPLE_ACCOUNT_ID"}, Default: "14", WriteKey: "EXAMPLE_ACCOUNT_ID"}}, true); err != nil {
	...
	if got, want := values["EXAMPLE_ACCOUNT_ID"], "14"; got != want {
		t.Fatalf("EXAMPLE_ACCOUNT_ID = %q, want %q", got, want)
	}
}
